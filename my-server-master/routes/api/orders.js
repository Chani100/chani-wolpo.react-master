...
router.patch("/menuOrder/:id", authmw, async (req, res) => {
  try {
    ...
    if (cardLike.OrderMenu.find((cardId) => cardId == req.body.card_id)) {
      const cardFiltered = cardLike.OrderMenu.filter(
        (cardId) => cardId != req.body.card_id
      );
      ...
      cardLike.OrderMenu = [...cardLike.OrderMenu, req.body.card_id];
      cardLike = await cardLike.save();
      // res.json({ msg: "The card has been removed rom the favorites list." });
    }
    console.log(cardLike.OrderMenu);
    res.json(cardLike);
  } catch (err) {
