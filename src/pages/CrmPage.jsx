...
const CRMPage = () => {
  ...
  const getOrdersTable = async () => {
    try {
      const tableOrders = await axios.get("/bookAtable");
  ...
  const handleUserDelete = async (id) => {
    try {
      ...
      setUsers((newUserssArr) => newUserssArr.filter((item) => item._id != id));
      toast.success("The user has been deleted from the system");
    } catch (err) {
      toast.error("Aggravation The user was not deleted");
    }
  };
  const handleUserAdminEdit = async (id) => {
    try {
      await axios.patch("/users/" + id);
         window.location.reload();
      toast.success("The change was made successfully");
    } catch (err) {
      toast.error("The change was not made successfully");
    }
  };
    ...
    <Container>
      ...
      <Nav variant="underline" defaultActiveKey="/home" className="nav_catgory">
        <Nav.Item className="nav_item_catgory">
          <Nav.Link eventKey="link-1" onClick={handelAll}>
            All
          </Nav.Link>
        </Nav.Item>
        <Nav.Item className="nav_item_catgory">
